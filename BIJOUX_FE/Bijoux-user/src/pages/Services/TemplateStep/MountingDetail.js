...
export default function MountingDetail() {
    ...
    const [mainMetal, setMainMetal] = useState(finalProduct ? finalProduct.metal_1 : null); //biến state lưu id của kim loại Main
    const [sideMetal, setSideMetal] = useState(finalProduct ? finalProduct.metal_2 : null); //biến state lưu id của kim loại Side
    const [fingerSize, setFingerSize] = useState(finalProduct ? finalProduct.mounting_size : null); //biến state lưu Finger Size
    const [checkMountingType, setCheckMountingType] = useState(model.mounting_type.id === 3 ? false : true); //biến state đóng mở chức năng chọn Finger Size
    const [sideMetalListVisible, setSideMetalListVisible] = useState(finalProduct !== null && model.model_metal[1]); //biến state đóng mở chức năng chọn Side Metal
    const MainMetalArray = model.model_metal.filter(metal => metal.is_main); //lấy mảng model_metal Main
    const rangeArray = generateRange(3, 10); //lấy mảng Size
    const SideMetalArray = model.model_metal.filter(metal => !metal.is_main); //lấy mảng model_metal Side
    ...
    const handleMainMetalChange = (event) => {
        ...
        { model.model_metal[0] ? setSideMetalListVisible(true) : setSideMetalListVisible(false) }
    }

    console.log("Main metal nè: ", mainMetal);
    console.log("Side metal nè: ", sideMetal)
    ...
    const handleSelectModel = () => {
        ...
        window.location.href = '/template?step=2&mountingType=' + model.mounting_type.id + '&model_id=' + id; //CHUYỂN HƯỚNG SANG TRANG CHỌN KIM CƯƠNG
    }
    useEffect(() => {
        const mountingTypeQuery = query.get("mountingType");

        if (index && index.id) {
            const model_id = index_id;
    ...
    //useEffect để thay đổi các options của kim loại Side khi kim loại Main thay đổi
    useEffect(() => {
        if (mainMetal !== null) {
            const filteredSideMetals = SideMetalArray.flatMap(sideMetalObj => {
                return sideMetalObj.metal.filter(metal => {
                    switch (mainMetal.id) {
                        case 1:
                            return metal.id !== 3;
                        case 2:
            ...
            setSideMetalArrayAfter(filteredSideMetals);
        }
    }, [mainMetal]);
            ...
            <div className="grid grid-cols-2 gap-5 w-full h-auto">
                ...
                <div className="flex flex-col w-full">
                    ...
                    <FormControl className="w-1/3">
                        ...
                        >
                            {MainMetalArray[0].metal.map((metal) => (

                                <MenuItem key={metal.id} value={JSON.stringify(metal)}>
                                    <img
